...
public class JdbcTemplateReviewRepository implements ReviewRepository{
    ...
    public List<Review> findReviews(Category category, Filter filter){
        String findQuery = "select r.reviewIdx, r.userIdx, r.title, r.contents, r.img_path from review as r " +
                           "join (select u.userIdx, u.name, u.gender, u.height, u.weight from user as u)"+
                           "U on U.userIdx = r.userIdx and U.weight = ? and U.height = ?" +
                           "where r.sports = ? " +
                           "and r.part = ? " +
                           "and r.brand = ? ";
        Object[] findQueryParam = new Object[]{filter.getWeight(), filter.getHeight(), category.getSportsAsInt(), category.getPartAsInt(), category.getBrandAsInt()};
        return jdbcTemplate.query(findQuery, postRowMapper(), findQueryParam);
    }

    private RowMapper<Review> postRowMapper() {
        return (rs, rowNum) -> {
            Review review = new Review();
            review.setReviewIdx(rs.getLong("reviewIdx"));
